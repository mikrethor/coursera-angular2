...
export class MenuSearchService {
  ...
  getAllCategories(): Observable<Array> {

    return this.http.get(this.constantService.API_ENDPOINT + "/categories.json")
      .flatMap((response) => response.json());

  }


  getItemsForCategory(categoryShortName: string): Observable<Array> {

    return this.http.get(this.constantService.API_ENDPOINT + "/menu_items.json?category=" + categoryShortName)
      .flatMap((response) => response.json()['menu_items']);
  }
}
